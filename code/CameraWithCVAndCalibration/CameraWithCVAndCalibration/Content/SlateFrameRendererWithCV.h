...
namespace BasicHologram
{
    ...
    public:
        SlateFrameRendererWithCV(std::shared_ptr<DX::DeviceResources> const& deviceResources,
                                 std::function<void(IResearchModeSensorFrame* pSensorFrame, cv::Mat& cvResultMat, cv::Mat& cvResultMask, std::vector<int> &ids, std::vector<std::vector<cv::Point2f>> &corners)> cvFrameProcessor) :
        ...
        void UpdateSlateTextureWithBitmapMask(const BYTE* pImage, const BYTE* pImageMask, UINT uWidth, UINT uHeight);
        void UpdateSlateTextureWithAHATBitmap(const UINT16* pDepth, UINT uWidth, UINT uHeight);
    ...
    protected:
        ...
        std::function<void(IResearchModeSensorFrame* pSensorFrame, cv::Mat& cvResultMat, cv::Mat& cvResultMask, std::vector<int> &ids, std::vector<std::vector<cv::Point2f>> &corners)> m_cvFrameProcessor;

        cv::Mat m_cvResultMat;
        cv::Mat m_cvResultMask;
    };
}
