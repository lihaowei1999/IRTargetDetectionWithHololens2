...
void SlateCameraRenderer::EnsureTextureForCameraFrame(IResearchModeSensorFrame* pSensorFrame)
{
    ...
    {
        if (m_pRMCameraSensor->GetSensorType() == DEPTH_AHAT) {
    ...
    if (pDepthFrame)
    {
        ...
        for (UINT i = 0; i < resolution.Height; i++)
        {
            for (UINT j = 0; j < resolution.Width; j++)
            {
                ...
                *((UINT32*)(mappedTexture)+((m_texture2D->GetRowPitch() / 4) * i + (resolution.Width - j - 1))) = pixel;
            }
        }
        ...
        if (m_pRMCameraSensor->GetSensorType() == DEPTH_AHAT)
        {
            ...
            BYTE input_max = (BYTE)255;
            UINT32 PIXEL_MAX = input_max | (input_max << 8) | (input_max << 16);
            cv::Mat processed(resolution.Height, resolution.Width, CV_8U, (void*)pAbImage);
